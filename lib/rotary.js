...
function Rotary(opt) {
    var _DEFAULT = {
        ...
        divide: 8,//转盘奖品划分的份数,7个奖品对应的索引值就是0~7,索引从0度开始
    ...
    this.start = function () {
        ...
        createjs.Ticker.addEventListener("tick", panTick);
        createjs.Ticker.timingMode = createjs.Ticker.RAF;
        createjs.Ticker.setFPS(_DEFAULT.fps || 60);

        function handleTick() {
            if (pin_zhen.status === 2) {
                // debugger;
                var ds = pin_zhen.target - pin_zhen.angle;
                pin_zhen.step = ds * pin_zhen.damping;
                if (pin_zhen.step < 0.001) {
                    if (successCallback) {
                        successCallback();
            ...
            var next_pin_zhen = _this._calcPos(pin_zhen.angle + pin_zhen.step, pin_zhen.step, pin_zhen.status);
            if (pin_zhen.hasOwnProperty('target')) {
                next_pin_zhen.target = pin_zhen.target;
            }
            if (pin_zhen.hasOwnProperty('damping')) {
                next_pin_zhen.damping = pin_zhen.damping;
            }
    ...
    this.endTo = function (target, callback) {
        // debugger
        pin_zhen.status = 2;
        pin_zhen.target = pin_zhen.angle + (Math.PI * 2 * _DEFAULT.pin_end - pin_zhen.angle % (Math.PI * 2)) + Math.PI * 2 / 8 * target;
        pin_zhen.damping = _DEFAULT.pin_step / (pin_zhen.target - pin_zhen.angle);
